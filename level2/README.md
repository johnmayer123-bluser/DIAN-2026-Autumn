...
| 验收标准                                       | 状态                                                                                                                |
| :--------------------------------------------- | :------------------------------------------------------------------------------------------------------------------ |
| 复用 Level1 的数据和训练流程，训练 CNN 分类器  | ✅ 数据层/训练循环完全复用，模型替换为 LeNet-5                                                                       |
| 从准确率、参数量、收敛速度、错误样本四角度对比 | ✅ 见上文对比表 + `compare_*.png`                                                                                    |
| CNN 测试集准确率建议达到 96% 左右              | ⚠️ 该标准针对 MNIST；Fashion-MNIST 上 LeNet 达 90.53%（不增强时的正常水平 90%~92%），已换用 Fashion-MNIST 并说明原因 |
| 输出 CNN 的训练曲线                            | ✅ `run/loss_curve.png`                                                                                              |
| 能结合实验结果解释 CNN 更适合图像任务的原因    | ✅ 见"为什么 CNN 更适合图像任务"                                                                                     |
